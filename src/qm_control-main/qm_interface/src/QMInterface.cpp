...
std::unique_ptr<StateInputCost> QMInterface::getJointLimitSoftConstraint(const ocs2::PinocchioInterface &pinocchioInterface,
    ...
    const int armStateDim = 7;
    const int armInputDim = 7;
    const int armStateIdx = centroidalModelInfo_.stateDim - 7;
    const int armInputIdx = centroidalModelInfo_.inputDim - 7;
...
matrix_t QMInterface::initializeInputCostWeight(const std::string &taskFile, const ocs2::CentroidalModelInfo &info) {
    ...
    for (size_t i = 0; i < info.numThreeDofContacts; i++) {
        ...
        base2feetJac.block(3 * i, 0, 3, info.actuatedDofNum-7) = jac.block(0, 6, 3, info.actuatedDofNum-7);
    }
    ...
    r.block(totalContactDim, totalContactDim, info.actuatedDofNum-7, info.actuatedDofNum-7) =
            base2feetJac.transpose() * rTaskspace.block(totalContactDim, totalContactDim, info.actuatedDofNum-7, info.actuatedDofNum-7)
            * base2feetJac;
