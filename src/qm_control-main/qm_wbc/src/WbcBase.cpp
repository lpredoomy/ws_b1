      ...
      armEeKinematics_(armEeKinematics.clone())
{
    ...
    jointKp_ = matrix_t::Zero(7, 7);
    jointKd_ = matrix_t::Zero(7, 7);
    d_arm_ = vector_t::Zero(7);
...
void WbcBase::dynamicCallback(qm_wbc::WbcWeightConfig &config, uint32_t) {
    ...
    jointKp_(6, 6) = config.kp_arm_joint_7;
    ...
    jointKd_(6, 6) = config.kd_arm_joint_7;
...
Task WbcBase::formulateTorqueLimitsTask() {
    ...
    f.segment<19>(0) -= hj;
    f.segment<19>(info_.actuatedDofNum) += hj;
...
{
    matrix_t a(7, numDecisionVars_);
    vector_t b(a.rows());

    a.setZero();
    b.setZero();

    a.block(0, info_.generalizedCoordinatesNum-7, 7, 7) = matrix_t::Identity(7, 7);

    matrix_t jointKp, jointKd;
    vector_t jointDesPos;
    jointKp = matrix_t::Zero(7, 7);
    jointKd = matrix_t::Zero(7, 7);
    jointDesPos = vector_t::Zero(7, 7);
    ...
    b = jointKp * (qDesired_.segment<7>(info_.generalizedCoordinatesNum-7)
            - qMeasured_.segment<7>(info_.generalizedCoordinatesNum-7))
        + jointKd * (vDesired_.segment<7>(info_.generalizedCoordinatesNum-7)
                - vMeasured_.segment<7>(info_.generalizedCoordinatesNum-7));
...
Task WbcBase::formulatejointDampTrackingTask()
{
    ...
    scalar_t damp = 0.001;
    a.block(0, info_.generalizedCoordinatesNum-7, 7, 7) = damp * matrix_t::Identity(7, 7);
...
void WbcBase::loadTasksSetting(const std::string &taskFile, bool verbose) {
    ...
    // arm torque limit
    armTorqueLimits_ = vector_t(7);
    armTorqueLimits_ = pinocchioInterfaceMeasured_.getModel().effortLimit.tail(7);
